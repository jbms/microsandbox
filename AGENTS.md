...
By explicit user direction, the SDK/runtime compatibility window follows the implementation candidate: a 0.N.x candidate must support existing workflows with SDKs and runtimes from 0.N.x and 0.(N-1).x in both directions. Derive the window from the candidate version, including on bump PRs, not the globally latest published release. For example, 0.7.x covers 0.7.x and 0.6.x; 0.8.x covers 0.8.x and 0.7.x. There is no permanent 0.6.0 floor or 0.6.18 baseline. This supersedes the earlier fixed historical window. CI patch sampling is not the support policy: representative passes do not establish coverage of every patch. Preserve older results as diagnostic evidence; this scope decision does not remove existing codecs or capability gates. The known v0.6.9 ↔ v0.6.10 exception remains unchanged when applicable.
